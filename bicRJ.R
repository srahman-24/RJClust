...
emstep = function(y, C, mu, Sigma, prob, iter.max, N, ...)
{
  ...
  for(ss in 1:iter.max)
  {
    ...
    loglik.diff     = abs(loglik_new - loglik.G(y, z, mu, Sigma,C)) 
    if(loglik.diff< 1e-6) 
    {break}
    else  
    {
      loglik_new  = loglik.G(y, z, mu, Sigma,C)
    }
    #print(Sigma)
  } 
...
bic.G = function(C,y, mu, Sigma, N, prob, iter.max,...)
{
  ...
  return(list(bic.value = 2 * loglik.G(y, z, mu, Sigma,C) - (nparams.G(C,nrow(y)) * log(N)), z = z, Sigma = Sigma, mu = mu))
