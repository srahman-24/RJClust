...
emstep = function(y, C, mu, Sigma, prob, iter.max, N, ...)
{
  ...
  for(ss in 1:iter.max)
  {
    ...
    for(ii in 1:N)
    {
      ...
      z[ii,] = t(rmultinom(1, 1, weights_multi(GG_new[ii,], prob, mu, Sigma,C)))
    }
    ...
    loglik.diff     = abs(loglik_new - loglik.G(GG_new, z, mu, Sigma,C)) 
    if(loglik.diff< 1e-6) 
    {break}
    else  
    {
      loglik_new  = loglik.G(GG_new, z, mu, Sigma,C)
    }
    #print(Sigma)
  } 
