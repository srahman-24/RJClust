...
for(ii in 1:(n/2))
{
  for(jj in 1:2)
  {
    X[ii,jj]          =   rnorm(1, 5, sigma)
    X[ii, (jj+2)]     =   rnorm(1, 1.5, sigma)
    X[(ii+5), (jj+2)] =   rnorm(1, 1, sigma)
  }
}
n1 = n2 = 5
...
for(kk1 in 1:C)
  { 
  aa = 0
  mu_diag_gibbs[kk1]   =   rnorm(1, sum(diag(GG)*(gamma_gibbs[,kk1])^2)/sum((gamma_gibbs[,kk1])^2), sigma[kk1,kk1]/sqrt(sum(gamma_gibbs[,kk1])^2))
  #aa                   =   sum((diag(GG) - mu_diag[kk1])^2*(gamma_gibbs[,kk1])^2)/2 
  for(kk2 in 1:C)
  { 
    cc = dd = 0
    for(ii in 1:(n-1))
    { 
      for(jj in (ii+1):n)
      {
           ...
           mu_offdiag_gibbs[kk1,kk2]  =   rnorm(1,  cc/dd, sigma[kk1,kk2]/sqrt(dd) )
    }
}
...
for(kk1 in 1:C)
{
  ...
  sigma[kk1,kk1]   = sqrt(1/rgamma(1, shape = t(gamma_gibbs[,kk1])%*%MM2%*%gamma_gibbs[,kk1] + 8, rate = t(gamma_gibbs[,kk1])%*%MM1%*%gamma_gibbs[,kk1] + (2.5/p)))
  for(kk2 in 1:C)     ##### for the offdiagonals of sigma ######                 
  {
    ...
    sigma[kk1,kk2] = sqrt(1/rgamma(1, shape = t(gamma_gibbs[,kk1])%*%MM2%*%gamma_gibbs[,kk2] + 8, rate = 0.5*(t(gamma_gibbs[,kk1])%*%MM1%*%gamma_gibbs[,kk2] + (2.5/p)))) 
  }
}
...
############# Gibbs steps for each gamma probability ############
WW  = matrix(0,nrow = n, ncol = C)
alpha1 = 2; alpha2 = 1
...
for(ii in 1:n)
{
  ...
  {         
              A2 = 0; A3 = 0; 
              A1 = 0.5*(log((sigma[kk,kk])^2) + (diag(GG)[ii] - mu_diag[kk])^2/(sigma[kk,kk])^2)
    for(jj in 1:n)
    {         if(jj!=ii)
              {A2 = A2 + 0.5*gamma_gibbs[jj,kk]*(log((sigma[kk,kk])^2/2) + (GG[ii,jj] - mu_offdiag[kk,kk])^2/((sigma[kk,kk])^2/2))}
      for(ll in 1:C)
      {
              if(ll!=kk)
              {  A3 = A3 + 0.5*gamma_gibbs[jj,ll]*(log((sigma[kk,ll])^2) + (GG[ii,jj] - mu_offdiag[kk,ll])^2/(sigma[kk,ll])^2)  }
      }
    }
              WW[ii,kk]  = rgamma(1, alpha1, alpha2 + A1 + A2+ A3)
  }
              WW[ii,]    = WW[ii,]/sum(WW[ii,])
}

gamma_gibbs    =    WW
GAMMA[[ss]]    =   gamma_gibbs
