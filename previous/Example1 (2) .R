...
for(kk1 in 1:C){  
  mu_diag_gibbs[kk1]   =   rnorm(1, sum(diag(G)*(gamma_gibbs[,kk1])^2)/sum(gamma_gibbs[,kk1])^2, sqrt(sigma_gibbs[kk1,kk1]^2/sum(gamma_gibbs[,kk1])^2))
  for(kk2 in (kk1+1):C)
  { cc = dd = 0
    for(ii in 1:(n-1))
    { 
...
#Check the condition  
if(mu_penalty(mu_diag, mu_offdiag, delta) == 1) 
